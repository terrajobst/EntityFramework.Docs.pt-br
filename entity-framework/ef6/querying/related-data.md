...
> [!NOTE]
> No momento, não é possível filtrar quais entidades relacionadas são carregadas. Include sempre trará todas as entidades relacionadas.  
...
> [!NOTE]
> O método de referência deve ser usado quando uma entidade tiver uma propriedade de navegação para outra entidade única. Por outro lado, o método de coleção deve ser usado quando uma entidade tem uma propriedade de navegação para uma coleção de outras entidades.
...
> [!NOTE]
> Embora a relação possa ser especificada como uma cadeia de caracteres em vez de uma expressão lambda, o IQueryable retornado não é genérico quando uma cadeia de caracteres é usada e, portanto, o método Cast geralmente é necessário antes que qualquer coisa útil possa ser feito com ele.
