...
### <a name="the-qualities-of-testable-code"></a><span data-ttu-id="660d1-126">As qualidades do código de teste</span><span class="sxs-lookup"><span data-stu-id="660d1-126">The Qualities of Testable Code</span></span>
...
### <a name="the-unit-of-work-pattern"></a><span data-ttu-id="660d1-189">O padrão de unidade de trabalho</span><span class="sxs-lookup"><span data-stu-id="660d1-189">The Unit of Work Pattern</span></span>
...
<span data-ttu-id="660d1-275">Uma implementação de unidade de trabalho falsa que podemos usar para teste pode ser semelhante ao seguinte.</span><span class="sxs-lookup"><span data-stu-id="660d1-275">A fake unit of work implementation that we can use for testing might look like the following.</span></span>
...
<span data-ttu-id="660d1-388">Também há uma pequena alteração que precisamos fazer dentro de nossa unidade de trabalho.</span><span class="sxs-lookup"><span data-stu-id="660d1-388">There is also one minor change we need to make inside our unit of work.</span></span> <span data-ttu-id="660d1-389">O carregamento lento é *desativado* por padrão ao trabalhar diretamente com um objeto ObjectContext.</span><span class="sxs-lookup"><span data-stu-id="660d1-389">Lazy loading is *off* by default when working directly with an ObjectContext object.</span></span> <span data-ttu-id="660d1-390">Há uma propriedade que podemos definir na propriedade ContextOptions para habilitar o carregamento adiado e podemos definir essa propriedade dentro de nossa unidade real de trabalho se quisermos habilitar o carregamento lento em todos os lugares.</span><span class="sxs-lookup"><span data-stu-id="660d1-390">There is a property we can set on the ContextOptions property to enable deferred loading, and we can set this property inside our real unit of work if we want to enable lazy loading everywhere.</span></span>
...
<span data-ttu-id="660d1-392">O carregamento lento torna o código do aplicativo mais fácil de escrever e, com a mágica do proxy, o código permanece completamente testado.</span><span class="sxs-lookup"><span data-stu-id="660d1-392">Lazy loading makes the application code easier to write, and with the proxy magic the code remains completely testable.</span></span> <span data-ttu-id="660d1-393">As falsificações na memória da unidade de trabalho podem simplesmente pré-carregar entidades falsas com dados associados quando necessário durante um teste.</span><span class="sxs-lookup"><span data-stu-id="660d1-393">In-memory fakes of the unit of work can simply preload fake entities with associated data when needed during a test.</span></span>

<span data-ttu-id="660d1-394">Neste ponto, vamos transformar nossa atenção na criação de repositórios usando IObjectSet&lt;T&gt; e examinar abstrações para ocultar todos os sinais da estrutura de persistência.</span><span class="sxs-lookup"><span data-stu-id="660d1-394">At this point we’ll turn our attention from building repositories using IObjectSet&lt;T&gt; and look at abstractions to hide all signs of the persistence framework.</span></span>

## <a name="custom-repositories"></a><span data-ttu-id="660d1-395">Repositórios personalizados</span><span class="sxs-lookup"><span data-stu-id="660d1-395">Custom Repositories</span></span>

<span data-ttu-id="660d1-396">Quando apresentamos inicialmente o padrão de design de unidade de trabalho neste artigo, fornecemos um código de exemplo para o que seria a unidade de trabalho.</span><span class="sxs-lookup"><span data-stu-id="660d1-396">When we first presented the unit of work design pattern in this article we provided some sample code for what the unit of work might look like.</span></span> <span data-ttu-id="660d1-397">Vamos apresentar essa ideia original usando o cenário de cartão de horário de funcionário e funcionário com o qual trabalhamos.</span><span class="sxs-lookup"><span data-stu-id="660d1-397">Let’s re-present this original idea using the employee and employee time card scenario we’ve been working with.</span></span>
...
<span data-ttu-id="660d1-398">A principal diferença entre essa unidade de trabalho e a unidade de trabalho que criamos na última seção é como essa unidade de trabalho não usa nenhuma abstração da estrutura EF4 (não há IObjectSet&lt;T&gt;).</span><span class="sxs-lookup"><span data-stu-id="660d1-398">The primary difference between this unit of work and the unit of work we created in the last section is how this unit of work does not use any abstractions from the EF4 framework (there is no IObjectSet&lt;T&gt;).</span></span> <span data-ttu-id="660d1-399">IObjectSet&lt;T&gt; funciona bem como uma interface de repositório, mas a API que ele expõe pode não alinhar perfeitamente com as necessidades do nosso aplicativo.</span><span class="sxs-lookup"><span data-stu-id="660d1-399">IObjectSet&lt;T&gt; works well as a repository interface, but the API it exposes might not perfectly align with our application’s needs.</span></span> <span data-ttu-id="660d1-400">Nesta próxima abordagem, representaremos repositórios usando uma abstração personalizada de IRepository&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-400">In this upcoming approach we will represent repositories using a custom IRepository&lt;T&gt; abstraction.</span></span>

<span data-ttu-id="660d1-401">Muitos desenvolvedores que seguem o design controlado por testes, design controlado por comportamento e metodologias orientadas a domínio design preferem a abordagem IRepository&lt;T&gt; por vários motivos.</span><span class="sxs-lookup"><span data-stu-id="660d1-401">Many developers who follow test-driven design, behavior-driven design, and domain driven methodologies design prefer the IRepository&lt;T&gt; approach for several reasons.</span></span> <span data-ttu-id="660d1-402">Primeiro, a interface IRepository&lt;T&gt; representa uma camada de "anticorrupção".</span><span class="sxs-lookup"><span data-stu-id="660d1-402">First, the IRepository&lt;T&gt; interface represents an “anti-corruption” layer.</span></span> <span data-ttu-id="660d1-403">Conforme descrito por Eric Evans em seu livro de design controlado por domínio, uma camada anticorrupção mantém seu código de domínio longe das APIs de infraestrutura, como uma API de persistência.</span><span class="sxs-lookup"><span data-stu-id="660d1-403">As described by Eric Evans in his Domain Driven Design book an anti-corruption layer keeps your domain code away from infrastructure APIs, like a persistence API.</span></span> <span data-ttu-id="660d1-404">Em segundo lugar, os desenvolvedores podem criar métodos no repositório que atendam às necessidades exatas de um aplicativo (como descoberto durante a gravação de testes).</span><span class="sxs-lookup"><span data-stu-id="660d1-404">Secondly, developers can build methods into the repository that meet the exact needs of an application (as discovered while writing tests).</span></span> <span data-ttu-id="660d1-405">Por exemplo, em geral, podemos precisar localizar uma única entidade usando um valor de ID, para que possamos adicionar um método FindById à interface do repositório.</span><span class="sxs-lookup"><span data-stu-id="660d1-405">For example, we might frequently need to locate a single entity using an ID value, so we can add a FindById method to the repository interface.</span></span><span data-ttu-id="660d1-406">  Nossa definição IRepository&lt;T&gt; será parecida com a seguinte.</span><span class="sxs-lookup"><span data-stu-id="660d1-406">  Our IRepository&lt;T&gt; definition will look like the following.</span></span>
...
<span data-ttu-id="660d1-407">Observe que vamos voltar para usando uma interface IQueryable&lt;T&gt; para expor coleções de entidades.</span><span class="sxs-lookup"><span data-stu-id="660d1-407">Notice we’ll drop back to using an IQueryable&lt;T&gt; interface to expose entity collections.</span></span> <span data-ttu-id="660d1-408">IQueryable&lt;T&gt; permite que as árvores de expressões LINQ fluam para o provedor EF4 e fornecem ao provedor uma visão holística da consulta.</span><span class="sxs-lookup"><span data-stu-id="660d1-408">IQueryable&lt;T&gt; allows LINQ expression trees to flow into the EF4 provider and give the provider a holistic view of the query.</span></span> <span data-ttu-id="660d1-409">Uma segunda opção seria retornar IEnumerable&lt;T&gt;, o que significa que o provedor do EF4 LINQ só verá as expressões criadas dentro do repositório.</span><span class="sxs-lookup"><span data-stu-id="660d1-409">A second option would be to return IEnumerable&lt;T&gt;, which means the EF4 LINQ provider will only see the expressions built inside of the repository.</span></span> <span data-ttu-id="660d1-410">Qualquer agrupamento, ordenação e projeção feitos fora do repositório não será composto no comando SQL enviado ao banco de dados, o que pode prejudicar o desempenho.</span><span class="sxs-lookup"><span data-stu-id="660d1-410">Any grouping, ordering, and projection done outside of the repository will not be composed into the SQL command sent to the database, which can hurt performance.</span></span> <span data-ttu-id="660d1-411">Por outro lado, um repositório que retorna apenas IEnumerable&lt;T&gt; resultados nunca surpreenderá você com um novo comando SQL.</span><span class="sxs-lookup"><span data-stu-id="660d1-411">On the other hand, a repository returning only IEnumerable&lt;T&gt; results will never surprise you with a new SQL command.</span></span> <span data-ttu-id="660d1-412">Ambas as abordagens funcionarão e ambas as abordagens permanecerão em teste.</span><span class="sxs-lookup"><span data-stu-id="660d1-412">Both approaches will work, and both approaches remain testable.</span></span>

<span data-ttu-id="660d1-413">É simples fornecer uma única implementação da interface IRepository&lt;T&gt; usando genéricos e a API ObjectContext do EF4.</span><span class="sxs-lookup"><span data-stu-id="660d1-413">It’s straightforward to provide a single implementation of the IRepository&lt;T&gt; interface using generics and the EF4 ObjectContext API.</span></span>
...
<span data-ttu-id="660d1-414">A abordagem IRepository&lt;T&gt; nos dá algum controle adicional sobre nossas consultas, pois um cliente precisa invocar um método para chegar a uma entidade.</span><span class="sxs-lookup"><span data-stu-id="660d1-414">The IRepository&lt;T&gt; approach gives us some additional control over our queries because a client has to invoke a method to get to an entity.</span></span> <span data-ttu-id="660d1-415">Dentro do método, poderíamos fornecer verificações adicionais e operadores LINQ para impor restrições de aplicativo.</span><span class="sxs-lookup"><span data-stu-id="660d1-415">Inside the method we could provide additional checks and LINQ operators to enforce application constraints.</span></span> <span data-ttu-id="660d1-416">Observe que a interface tem duas restrições no parâmetro de tipo genérico.</span><span class="sxs-lookup"><span data-stu-id="660d1-416">Notice the interface has two constraints on the generic type parameter.</span></span> <span data-ttu-id="660d1-417">A primeira restrição é a classe contras que os&gt;&lt;contras de IEntity e a segunda restrição forçam as entidades a implementarem a criação de uma ou uma abstração criada para o aplicativo.</span><span class="sxs-lookup"><span data-stu-id="660d1-417">The first constraint is the class cons taint required by ObjectSet&lt;T&gt;, and the second constraint forces our entities to implement IEntity – an abstraction created for the application.</span></span> <span data-ttu-id="660d1-418">A interface IEntity força as entidades a terem uma propriedade de ID legível e, em seguida, podemos usar essa propriedade no método FindById.</span><span class="sxs-lookup"><span data-stu-id="660d1-418">The IEntity interface forces entities to have a readable Id property, and we can then use this property in the FindById method.</span></span> <span data-ttu-id="660d1-419">IEntity é definido com o código a seguir.</span><span class="sxs-lookup"><span data-stu-id="660d1-419">IEntity is defined with the following code.</span></span>
...
<span data-ttu-id="660d1-420">IEntity poderia ser considerado uma pequena violação do ignorância de persistência, já que nossas entidades são necessárias para implementar essa interface.</span><span class="sxs-lookup"><span data-stu-id="660d1-420">IEntity could be considered a small violation of persistence ignorance since our entities are required to implement this interface.</span></span> <span data-ttu-id="660d1-421">Lembre-se de que a persistência ignorância é sobre as compensações e, para muitos, a funcionalidade do FindById superará a restrição imposta pela interface.</span><span class="sxs-lookup"><span data-stu-id="660d1-421">Remember persistence ignorance is about tradeoffs, and for many the FindById functionality will outweigh the constraint imposed by the interface.</span></span> <span data-ttu-id="660d1-422">A interface não tem impacto na capacidade de teste.</span><span class="sxs-lookup"><span data-stu-id="660d1-422">The interface has no impact on testability.</span></span>

<span data-ttu-id="660d1-423">A instanciação de um IRepository dinâmico&lt;T&gt; requer um ObjectContext EF4, portanto, uma implementação concreta da unidade de trabalho deve gerenciar a instanciação.</span><span class="sxs-lookup"><span data-stu-id="660d1-423">Instantiating a live IRepository&lt;T&gt; requires an EF4 ObjectContext, so a concrete unit of work implementation should manage the instantiation.</span></span>
...
### <a name="using-the-custom-repository"></a><span data-ttu-id="660d1-424">Usando o repositório personalizado</span><span class="sxs-lookup"><span data-stu-id="660d1-424">Using the Custom Repository</span></span>

<span data-ttu-id="660d1-425">Usar nosso repositório personalizado não é significativamente diferente de usar o repositório com base em IObjectSet&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-425">Using our custom repository is not significantly different from using the repository based on IObjectSet&lt;T&gt;.</span></span> <span data-ttu-id="660d1-426">Em vez de aplicar operadores LINQ diretamente a uma propriedade, primeiro precisaremos invocar os métodos do repositório para obter uma referência IQueryable&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-426">Instead of applying LINQ operators directly to a property, we’ll first need to invoke one the repository’s methods to grab an IQueryable&lt;T&gt; reference.</span></span>
...
<span data-ttu-id="660d1-427">Observe que o operador de inclusão personalizado que implementamos anteriormente funcionará sem alteração.</span><span class="sxs-lookup"><span data-stu-id="660d1-427">Notice the custom Include operator we implemented previously will work without change.</span></span> <span data-ttu-id="660d1-428">O método FindById do repositório remove a lógica duplicada das ações que tentam recuperar uma única entidade.</span><span class="sxs-lookup"><span data-stu-id="660d1-428">The repository’s FindById method removes duplicated logic from actions trying to retrieve a single entity.</span></span>
...
<span data-ttu-id="660d1-429">Não há nenhuma diferença significativa na capacidade de teste das duas abordagens que examinamos.</span><span class="sxs-lookup"><span data-stu-id="660d1-429">There is no significant difference in the testability of the two approaches we’ve examined.</span></span> <span data-ttu-id="660d1-430">Poderíamos fornecer implementações falsas de IRepository&lt;T&gt; criando classes concretas apoiadas por HashSet&lt;funcionário&gt;-exatamente como o que fizemos na última seção.</span><span class="sxs-lookup"><span data-stu-id="660d1-430">We could provide fake implementations of IRepository&lt;T&gt; by building concrete classes backed by HashSet&lt;Employee&gt; - just like what we did in the last section.</span></span> <span data-ttu-id="660d1-431">No entanto, alguns desenvolvedores preferem usar objetos fictícios e estruturas de objeto fictícios em vez de criar falsificações.</span><span class="sxs-lookup"><span data-stu-id="660d1-431">However, some developers prefer to use mock objects and mock object frameworks instead of building fakes.</span></span> <span data-ttu-id="660d1-432">Veremos como usar as simulações para testar nossa implementação e discutir as diferenças entre as simulações e as falsificações na próxima seção.</span><span class="sxs-lookup"><span data-stu-id="660d1-432">We’ll look at using mocks to test our implementation and discuss the differences between mocks and fakes in the next section.</span></span>

### <a name="testing-with-mocks"></a><span data-ttu-id="660d1-433">Teste com simulações</span><span class="sxs-lookup"><span data-stu-id="660d1-433">Testing with Mocks</span></span>

<span data-ttu-id="660d1-434">Há diferentes abordagens para criar o que Martin Fowler chama de "duplicata de teste".</span><span class="sxs-lookup"><span data-stu-id="660d1-434">There are different approaches to building what Martin Fowler calls a “test double”.</span></span> <span data-ttu-id="660d1-435">Um duplo de teste (como um filme stunt duplo) é um objeto que você cria para "entrar" para objetos reais, de produção durante os testes.</span><span class="sxs-lookup"><span data-stu-id="660d1-435">A test double (like a movie stunt double) is an object you build to “stand in” for real, production objects during tests.</span></span> <span data-ttu-id="660d1-436">Os repositórios na memória que criamos são duplicatas de teste para os repositórios que falam com SQL Server.</span><span class="sxs-lookup"><span data-stu-id="660d1-436">The in-memory repositories we created are test doubles for the repositories that talk to SQL Server.</span></span> <span data-ttu-id="660d1-437">Vimos como usar essas duplicatas de teste durante os testes de unidade para isolar o código e manter os testes em execução rápida.</span><span class="sxs-lookup"><span data-stu-id="660d1-437">We’ve seen how to use these test-doubles during the unit tests to isolate code and keep tests running fast.</span></span>

<span data-ttu-id="660d1-438">As duplicatas de teste que criamos têm implementações reais e em funcionamento.</span><span class="sxs-lookup"><span data-stu-id="660d1-438">The test doubles we’ve built have real, working implementations.</span></span> <span data-ttu-id="660d1-439">Nos bastidores, cada um armazena uma coleção concreta de objetos, e eles adicionarão e removerão objetos dessa coleção à medida que manipularmos o repositório durante um teste.</span><span class="sxs-lookup"><span data-stu-id="660d1-439">Behind the scenes each one stores a concrete collection of objects, and they will add and remove objects from this collection as we manipulate the repository during a test.</span></span> <span data-ttu-id="660d1-440">Alguns desenvolvedores gostam de criar suas duplicatas de teste dessa maneira – com código real e implementações de trabalho.</span><span class="sxs-lookup"><span data-stu-id="660d1-440">Some developers like to build their test doubles this way – with real code and working implementations.</span></span><span data-ttu-id="660d1-441">  Essas duplicatas de teste são o que chamamos de *falsificações*.</span><span class="sxs-lookup"><span data-stu-id="660d1-441">  These test doubles are what we call *fakes*.</span></span> <span data-ttu-id="660d1-442">Eles têm implementações de trabalho, mas não são reais o suficiente para uso em produção.</span><span class="sxs-lookup"><span data-stu-id="660d1-442">They have working implementations, but they aren’t real enough for production use.</span></span> <span data-ttu-id="660d1-443">Na verdade, o repositório falso não grava no banco de dados.</span><span class="sxs-lookup"><span data-stu-id="660d1-443">The fake repository doesn’t actually write to the database.</span></span> <span data-ttu-id="660d1-444">O servidor SMTP falso não envia realmente uma mensagem de email pela rede.</span><span class="sxs-lookup"><span data-stu-id="660d1-444">The fake SMTP server doesn’t actually send an email message over the network.</span></span>

### <a name="mocks-versus-fakes"></a><span data-ttu-id="660d1-445">Simulações versus falsificações</span><span class="sxs-lookup"><span data-stu-id="660d1-445">Mocks versus Fakes</span></span>

<span data-ttu-id="660d1-446">Há outro tipo de duplicata de teste conhecida como uma *simulação*.</span><span class="sxs-lookup"><span data-stu-id="660d1-446">There is another type of test double known as a *mock*.</span></span> <span data-ttu-id="660d1-447">Embora as falsificações tenham implementações em funcionamento, as simulações vêm sem implementação.</span><span class="sxs-lookup"><span data-stu-id="660d1-447">While fakes have working implementations, mocks come with no implementation.</span></span> <span data-ttu-id="660d1-448">Com a ajuda de uma estrutura de objetos fictício, construímos esses objetos fictícios em tempo de execução e os usamos como duplicatas de teste.</span><span class="sxs-lookup"><span data-stu-id="660d1-448">With the help of a mock object framework we construct these mock objects at run time and use them as test doubles.</span></span> <span data-ttu-id="660d1-449">Nesta seção, usaremos a estrutura de simulação de código-fonte aberto MOQ.</span><span class="sxs-lookup"><span data-stu-id="660d1-449">In this section we’ll be using the open source mocking framework Moq.</span></span> <span data-ttu-id="660d1-450">Veja um exemplo simples de como usar MOQ para criar dinamicamente um Double de teste para um repositório de funcionários.</span><span class="sxs-lookup"><span data-stu-id="660d1-450">Here is a simple example of using Moq to dynamically create a test double for an employee repository.</span></span>
...
<span data-ttu-id="660d1-451">Pedimos MOQ para um IRepository&lt;funcionário&gt; implementação e ele cria um dinamicamente.</span><span class="sxs-lookup"><span data-stu-id="660d1-451">We ask Moq for an IRepository&lt;Employee&gt; implementation and it builds one dynamically.</span></span> <span data-ttu-id="660d1-452">Podemos chegar ao objeto que implementa o IRepository&lt;Employee&gt; acessando a propriedade Object do objeto fictício&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-452">We can get to the object implementing IRepository&lt;Employee&gt; by accessing the Object property of the Mock&lt;T&gt; object.</span></span> <span data-ttu-id="660d1-453">É esse objeto interno que podemos passar para nossos controladores, e eles não saberão se esse é um duplo de teste ou o real Repository.</span><span class="sxs-lookup"><span data-stu-id="660d1-453">It is this inner object we can pass into our controllers, and they won’t know if this is a test double or the real repository.</span></span> <span data-ttu-id="660d1-454">Podemos invocar métodos no objeto da mesma forma que invocamos métodos em um objeto com uma implementação real.</span><span class="sxs-lookup"><span data-stu-id="660d1-454">We can invoke methods on the object just like we would invoke methods on an object with a real implementation.</span></span>

<span data-ttu-id="660d1-455">Você deve estar se perguntando o que o repositório de simulações fará quando invocarmos o método Add.</span><span class="sxs-lookup"><span data-stu-id="660d1-455">You must be wondering what the mock repository will do when we invoke the Add method.</span></span> <span data-ttu-id="660d1-456">Como não há nenhuma implementação por trás do objeto fictício, Add não faz nada.</span><span class="sxs-lookup"><span data-stu-id="660d1-456">Since there is no implementation behind the mock object, Add does nothing.</span></span> <span data-ttu-id="660d1-457">Não há nenhuma coleção concreta nos bastidores, como tivemos com as falsificações que escrevemos, portanto, o funcionário é Descartado.</span><span class="sxs-lookup"><span data-stu-id="660d1-457">There is no concrete collection behind the scenes like we had with the fakes we wrote, so the employee is discarded.</span></span> <span data-ttu-id="660d1-458">E quanto ao valor de retorno de FindById?</span><span class="sxs-lookup"><span data-stu-id="660d1-458">What about the return value of FindById?</span></span> <span data-ttu-id="660d1-459">Nesse caso, o objeto fictício faz a única coisa que ele pode fazer, que é retornar um valor padrão.</span><span class="sxs-lookup"><span data-stu-id="660d1-459">In this case the mock object does the only thing it can do, which is return a default value.</span></span> <span data-ttu-id="660d1-460">Como estamos retornando um tipo de referência (um funcionário), o valor de retorno é um valor nulo.</span><span class="sxs-lookup"><span data-stu-id="660d1-460">Since we are returning a reference type (an Employee), the return value is a null value.</span></span>

<span data-ttu-id="660d1-461">As simulações podem soar inúteis; no entanto, há mais dois recursos de simulações sobre os quais não falamos.</span><span class="sxs-lookup"><span data-stu-id="660d1-461">Mocks might sound worthless; however, there are two more features of mocks we haven’t talked about.</span></span> <span data-ttu-id="660d1-462">Primeiro, a estrutura MOQ registra todas as chamadas feitas no objeto fictício.</span><span class="sxs-lookup"><span data-stu-id="660d1-462">First, the Moq framework records all the calls made on the mock object.</span></span> <span data-ttu-id="660d1-463">Posteriormente, no código, podemos perguntar MOQ se alguém invocou o método Add ou se alguém invocou o método FindById.</span><span class="sxs-lookup"><span data-stu-id="660d1-463">Later in the code we can ask Moq if anyone invoked the Add method, or if anyone invoked the FindById method.</span></span> <span data-ttu-id="660d1-464">Veremos mais adiante como podemos usar esse recurso de gravação de "caixa preta" em testes.</span><span class="sxs-lookup"><span data-stu-id="660d1-464">We’ll see later how we can use this “black box” recording feature in tests.</span></span>

<span data-ttu-id="660d1-465">O segundo grande recurso é como podemos usar MOQ para programar um objeto fictício com *expectativas*.</span><span class="sxs-lookup"><span data-stu-id="660d1-465">The second great feature is how we can use Moq to program a mock object with *expectations*.</span></span> <span data-ttu-id="660d1-466">Uma expectativa diz ao objeto fictício como responder a qualquer interação determinada.</span><span class="sxs-lookup"><span data-stu-id="660d1-466">An expectation tells the mock object how to respond to any given interaction.</span></span> <span data-ttu-id="660d1-467">Por exemplo, podemos programar uma expectativa em nossa simulação e pedir que ela retorne um objeto Employee quando alguém invocar FindById.</span><span class="sxs-lookup"><span data-stu-id="660d1-467">For example, we can program an expectation into our mock and tell it to return an employee object when someone invokes FindById.</span></span> <span data-ttu-id="660d1-468">A estrutura MOQ usa uma API de instalação e expressões lambda para programar essas expectativas.</span><span class="sxs-lookup"><span data-stu-id="660d1-468">The Moq framework uses a Setup API and lambda expressions to program these expectations.</span></span>
...
<span data-ttu-id="660d1-469">Neste exemplo, solicitamos que o MOQ crie um repositório dinamicamente e, em seguida, programamos o repositório com uma expectativa.</span><span class="sxs-lookup"><span data-stu-id="660d1-469">In this sample we ask Moq to dynamically build a repository, and then we program the repository with an expectation.</span></span> <span data-ttu-id="660d1-470">A expectativa diz ao objeto fictício para retornar um novo objeto Employee com um valor de ID 5 quando alguém invoca o método FindById passando um valor de 5.</span><span class="sxs-lookup"><span data-stu-id="660d1-470">The expectation tells the mock object to return a new employee object with an Id value of 5 when someone invokes the FindById method passing a value of 5.</span></span> <span data-ttu-id="660d1-471">Esse teste passa, e não precisamos criar uma implementação completa para falsificar IRepository&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-471">This test passes, and we didn’t need to build a full implementation to fake IRepository&lt;T&gt;.</span></span>

<span data-ttu-id="660d1-472">Vamos revisitar os testes que escrevemos anteriormente e reutilizá-los para usar simulações em vez de falsificações.</span><span class="sxs-lookup"><span data-stu-id="660d1-472">Let’s revisit the tests we wrote earlier and rework them to use mocks instead of fakes.</span></span> <span data-ttu-id="660d1-473">Assim como antes, usaremos uma classe base para configurar as partes comuns da infraestrutura de que precisamos para todos os testes do controlador.</span><span class="sxs-lookup"><span data-stu-id="660d1-473">Just like before, we’ll use a base class to setup the common pieces of infrastructure we need for all of the controller’s tests.</span></span>
...
<span data-ttu-id="660d1-474">O código de instalação permanece basicamente o mesmo.</span><span class="sxs-lookup"><span data-stu-id="660d1-474">The setup code remains mostly the same.</span></span> <span data-ttu-id="660d1-475">Em vez de usar falsificações, usaremos MOQ para construir objetos fictícios.</span><span class="sxs-lookup"><span data-stu-id="660d1-475">Instead of using fakes, we’ll use Moq to construct mock objects.</span></span> <span data-ttu-id="660d1-476">A classe base organiza a unidade de simulação de trabalho para retornar um repositório fictício quando o código invoca a propriedade Employees.</span><span class="sxs-lookup"><span data-stu-id="660d1-476">The base class arranges for the mock unit of work to return a mock repository when code invokes the Employees property.</span></span> <span data-ttu-id="660d1-477">O restante da configuração de simulação ocorrerá dentro dos acessórios de teste dedicados a cada cenário específico.</span><span class="sxs-lookup"><span data-stu-id="660d1-477">The rest of the mock setup will take place inside the test fixtures dedicated to each specific scenario.</span></span> <span data-ttu-id="660d1-478">Por exemplo, o acessório de teste para a ação de índice configurará o repositório de simulações para retornar uma lista de funcionários quando a ação invocar o método FindAll do repositório fictício.</span><span class="sxs-lookup"><span data-stu-id="660d1-478">For example, the test fixture for the Index action will setup the mock repository to return a list of employees when the action invokes the FindAll method of the mock repository.</span></span>
...
<span data-ttu-id="660d1-479">Exceto pelas expectativas, nossos testes são semelhantes aos testes que tínhamos antes.</span><span class="sxs-lookup"><span data-stu-id="660d1-479">Except for the expectations, our tests look similar to the tests we had before.</span></span> <span data-ttu-id="660d1-480">No entanto, com a capacidade de gravação de uma estrutura fictícia, podemos abordar os testes de um ângulo diferente.</span><span class="sxs-lookup"><span data-stu-id="660d1-480">However, with the recording ability of a mock framework we can approach testing from a different angle.</span></span> <span data-ttu-id="660d1-481">Veremos essa nova perspectiva na próxima seção.</span><span class="sxs-lookup"><span data-stu-id="660d1-481">We’ll look at this new perspective in the next section.</span></span>

### <a name="state-versus-interaction-testing"></a><span data-ttu-id="660d1-482">Teste de Estado versus interação</span><span class="sxs-lookup"><span data-stu-id="660d1-482">State versus Interaction Testing</span></span>

<span data-ttu-id="660d1-483">Há técnicas diferentes que você pode usar para testar o software com objetos fictícios.</span><span class="sxs-lookup"><span data-stu-id="660d1-483">There are different techniques you can use to test software with mock objects.</span></span> <span data-ttu-id="660d1-484">Uma abordagem é usar o teste baseado em estado, que é o que fizemos neste documento até agora.</span><span class="sxs-lookup"><span data-stu-id="660d1-484">One approach is to use state based testing, which is what we have done in this paper so far.</span></span> <span data-ttu-id="660d1-485">O teste baseado em estado faz asserções sobre o estado do software.</span><span class="sxs-lookup"><span data-stu-id="660d1-485">State based testing makes assertions about the state of the software.</span></span> <span data-ttu-id="660d1-486">No último teste, invocamos um método de ação no controlador e fizemos uma asserção sobre o modelo que deveria criar.</span><span class="sxs-lookup"><span data-stu-id="660d1-486">In the last test we invoked an action method on the controller and made an assertion about the model it should build.</span></span> <span data-ttu-id="660d1-487">Aqui estão alguns exemplos de estado de teste:</span><span class="sxs-lookup"><span data-stu-id="660d1-487">Here are some other examples of testing state:</span></span>

-   <span data-ttu-id="660d1-488">Verifique se o repositório contém o novo objeto Employee após a criação de execute.</span><span class="sxs-lookup"><span data-stu-id="660d1-488">Verify the repository contains the new employee object after Create executes.</span></span>
-   <span data-ttu-id="660d1-489">Verifique se o modelo contém uma lista de todos os funcionários após a execução do índice.</span><span class="sxs-lookup"><span data-stu-id="660d1-489">Verify the model holds a list of all employees after Index executes.</span></span>
-   <span data-ttu-id="660d1-490">Verifique se o repositório não contém um determinado funcionário após a execução da exclusão.</span><span class="sxs-lookup"><span data-stu-id="660d1-490">Verify the repository does not contain a given employee after Delete executes.</span></span>

<span data-ttu-id="660d1-491">Outra abordagem que você verá com objetos fictícios é verificar as *interações*.</span><span class="sxs-lookup"><span data-stu-id="660d1-491">Another approach you’ll see with mock objects is to verify *interactions*.</span></span> <span data-ttu-id="660d1-492">Embora o teste baseado em estado faça declarações sobre o estado dos objetos, o teste baseado em interação faz asserções sobre como os objetos interagem.</span><span class="sxs-lookup"><span data-stu-id="660d1-492">While state based testing makes assertions about the state of objects, interaction based testing makes assertions about how objects interact.</span></span> <span data-ttu-id="660d1-493">Por exemplo:</span><span class="sxs-lookup"><span data-stu-id="660d1-493">For example:</span></span>

-   <span data-ttu-id="660d1-494">Verifique se o controlador invoca o método Add do repositório quando CREATE executa.</span><span class="sxs-lookup"><span data-stu-id="660d1-494">Verify the controller invokes the repository’s Add method when Create executes.</span></span>
-   <span data-ttu-id="660d1-495">Verifique se o controlador invoca o método FindAll do repositório quando o índice é executado.</span><span class="sxs-lookup"><span data-stu-id="660d1-495">Verify the controller invokes the repository’s FindAll method when Index executes.</span></span>
-   <span data-ttu-id="660d1-496">Verifique se o controlador invoca a unidade do método de confirmação do trabalho para salvar as alterações quando a edição é executada.</span><span class="sxs-lookup"><span data-stu-id="660d1-496">Verify the controller invokes the unit of work’s Commit method to save changes when Edit executes.</span></span>

<span data-ttu-id="660d1-497">O teste de interação geralmente requer menos dados de teste, porque não estamos na análise de coleções e verificando contagens.</span><span class="sxs-lookup"><span data-stu-id="660d1-497">Interaction testing often requires less test data, because we aren’t poking inside of collections and verifying counts.</span></span> <span data-ttu-id="660d1-498">Por exemplo, se soubermos que a ação detalhes invoca o método FindById do repositório com o valor correto, a ação provavelmente está se comportando corretamente.</span><span class="sxs-lookup"><span data-stu-id="660d1-498">For example, if we know the Details action invokes a repository’s FindById method with the correct value - then the action is probably behaving correctly.</span></span> <span data-ttu-id="660d1-499">Podemos verificar esse comportamento sem configurar dados de teste para retornar do FindById.</span><span class="sxs-lookup"><span data-stu-id="660d1-499">We can verify this behavior without setting up any test data to return from FindById.</span></span>
...
<span data-ttu-id="660d1-500">A única configuração necessária no acessório de teste acima é a configuração fornecida pela classe base.</span><span class="sxs-lookup"><span data-stu-id="660d1-500">The only setup required in the above test fixture is the setup provided by the base class.</span></span> <span data-ttu-id="660d1-501">Quando invocamos a ação do controlador, MOQ registrará as interações com o repositório fictício.</span><span class="sxs-lookup"><span data-stu-id="660d1-501">When we invoke the controller action, Moq will record the interactions with the mock repository.</span></span> <span data-ttu-id="660d1-502">Usando a API Verify de MOQ, podemos perguntar MOQ se o controlador invocava FindById com o valor de ID apropriado.</span><span class="sxs-lookup"><span data-stu-id="660d1-502">Using the Verify API of Moq, we can ask Moq if the controller invoked FindById with the proper ID value.</span></span> <span data-ttu-id="660d1-503">Se o controlador não invocar o método ou invocar o método com um valor de parâmetro inesperado, o método Verify gerará uma exceção e o teste falhará.</span><span class="sxs-lookup"><span data-stu-id="660d1-503">If the controller did not invoke the method, or invoked the method with an unexpected parameter value, the Verify method will throw an exception and the test will fail.</span></span>

<span data-ttu-id="660d1-504">Aqui está outro exemplo para verificar se a ação de criação invoca a confirmação na unidade de trabalho atual.</span><span class="sxs-lookup"><span data-stu-id="660d1-504">Here is another example to verify the Create action invokes Commit on the current unit of work.</span></span>
...
<span data-ttu-id="660d1-505">Um perigo do teste de interação é a tendência de especificar interações.</span><span class="sxs-lookup"><span data-stu-id="660d1-505">One danger with interaction testing is the tendency to over specify interactions.</span></span> <span data-ttu-id="660d1-506">A capacidade do objeto fictício de registrar e verificar cada interação com o objeto fictício não significa que o teste deve tentar verificar cada interação.</span><span class="sxs-lookup"><span data-stu-id="660d1-506">The ability of the mock object to record and verify every interaction with the mock object doesn’t mean the test should try to verify every interaction.</span></span> <span data-ttu-id="660d1-507">Algumas interações são detalhes de implementação e você só deve verificar as interações *necessárias* para atender ao teste atual.</span><span class="sxs-lookup"><span data-stu-id="660d1-507">Some interactions are implementation details and you should only verify the interactions *required* to satisfy the current test.</span></span>

<span data-ttu-id="660d1-508">A escolha entre simulações ou falsificações depende em grande parte do sistema que você está testando e suas preferências pessoais (ou de equipe).</span><span class="sxs-lookup"><span data-stu-id="660d1-508">The choice between mocks or fakes largely depends on the system you are testing and your personal (or team) preferences.</span></span> <span data-ttu-id="660d1-509">Os objetos fictícios podem reduzir drasticamente a quantidade de código de que você precisa para implementar duplicatas de teste, mas nem todos se sentem confortável em expectativas de programação e verificação de interações.</span><span class="sxs-lookup"><span data-stu-id="660d1-509">Mock objects can drastically reduce the amount of code you need to implement test doubles, but not everyone is comfortable programming expectations and verifying interactions.</span></span>

## <a name="conclusions"></a><span data-ttu-id="660d1-510">Conclusões</span><span class="sxs-lookup"><span data-stu-id="660d1-510">Conclusions</span></span>

<span data-ttu-id="660d1-511">Neste documento, demonstramos várias abordagens para criar um código que pode ser testado ao usar o Entity Framework ADO.NET para persistência de dados.</span><span class="sxs-lookup"><span data-stu-id="660d1-511">In this paper we’ve demonstrated several approaches to creating testable code while using the ADO.NET Entity Framework for data persistence.</span></span> <span data-ttu-id="660d1-512">Podemos aproveitar abstrações internas como IObjectSet&lt;T&gt;ou criar nossas próprias abstrações, como IRepository&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="660d1-512">We can leverage built in abstractions like IObjectSet&lt;T&gt;, or create our own abstractions like IRepository&lt;T&gt;.</span></span><span data-ttu-id="660d1-513">  Em ambos os casos, o suporte do POCO no ADO.NET Entity Framework 4,0 permite que os consumidores dessas abstrações permaneçam que desconhecem persistentes e altamente testados.</span><span class="sxs-lookup"><span data-stu-id="660d1-513">  In both cases, the POCO support in the ADO.NET Entity Framework 4.0 allows the consumers of these abstractions to remain persistent ignorant and highly testable.</span></span> <span data-ttu-id="660d1-514">Recursos de EF4 adicionais, como carregamento lento implícito, permitem que o código de negócios e de serviço de aplicativo funcione sem se preocupar com os detalhes de um armazenamento de dados relacional.</span><span class="sxs-lookup"><span data-stu-id="660d1-514">Additional EF4 features like implicit lazy loading allows business and application service code to work without worrying about the details of a relational data store.</span></span> <span data-ttu-id="660d1-515">Por fim, as abstrações que criamos são fáceis de simular ou falsas dentro dos testes de unidade, e podemos usar essas duplicatas de teste para obter testes de execução rápida, altamente isolados e confiáveis.</span><span class="sxs-lookup"><span data-stu-id="660d1-515">Finally, the abstractions we create are easy to mock or fake inside of unit tests, and we can use these test doubles to achieve fast running, highly isolated, and reliable tests.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="660d1-516">Recursos adicionais</span><span class="sxs-lookup"><span data-stu-id="660d1-516">Additional Resources</span></span>

-   <span data-ttu-id="660d1-517">Robert C. Martin, " [princípio de responsabilidade única](https://www.objectmentor.com/resources/articles/srp.pdf)"</span><span class="sxs-lookup"><span data-stu-id="660d1-517">Robert C. Martin, “ [The Single Responsibility Principle](https://www.objectmentor.com/resources/articles/srp.pdf)”</span></span>
-   <span data-ttu-id="660d1-518">Martin Fowler, [Catálogo de padrões](https://www.martinfowler.com/eaaCatalog/index.html) de *padrões de arquitetura de aplicativos empresariais*</span><span class="sxs-lookup"><span data-stu-id="660d1-518">Martin Fowler, [Catalog of Patterns](https://www.martinfowler.com/eaaCatalog/index.html) from *Patterns of Enterprise Application Architecture*</span></span>
-   <span data-ttu-id="660d1-519">Griffin Caprio, " [injeção de dependência](https://msdn.microsoft.com/magazine/cc163739.aspx)"</span><span class="sxs-lookup"><span data-stu-id="660d1-519">Griffin Caprio, “ [Dependency Injection](https://msdn.microsoft.com/magazine/cc163739.aspx)”</span></span>
-   <span data-ttu-id="660d1-520">Blog de programação de dados, " [passo a passos: desenvolvimento controlado por testes com o Entity Framework 4,0](https://blogs.msdn.com/adonet/pages/walkthrough-test-driven-development-with-the-entity-framework-4-0.aspx)".</span><span class="sxs-lookup"><span data-stu-id="660d1-520">Data Programmability Blog, “ [Walkthrough: Test Driven Development with the Entity Framework 4.0](https://blogs.msdn.com/adonet/pages/walkthrough-test-driven-development-with-the-entity-framework-4-0.aspx)”.</span></span>
-   <span data-ttu-id="660d1-521">Blog de programação de dados, " [usando o repositório e os padrões de unidade de trabalho com o Entity Framework 4,0](https://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)"</span><span class="sxs-lookup"><span data-stu-id="660d1-521">Data Programmability Blog, “ [Using Repository and Unit of Work patterns with Entity Framework 4.0](https://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)”</span></span>
-   <span data-ttu-id="660d1-522">Aaron Jensen, " [apresentando especificações de máquina](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)"</span><span class="sxs-lookup"><span data-stu-id="660d1-522">Aaron Jensen, “ [Introducing Machine Specifications](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)”</span></span>
-   <span data-ttu-id="660d1-523">Eric Lee, " [BDD com MSTest](https://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)"</span><span class="sxs-lookup"><span data-stu-id="660d1-523">Eric Lee, “ [BDD with MSTest](https://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)”</span></span>
-   <span data-ttu-id="660d1-524">Eric Evans, " [design controlado por domínio](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)"</span><span class="sxs-lookup"><span data-stu-id="660d1-524">Eric Evans, “ [Domain Driven Design](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)”</span></span>
-   <span data-ttu-id="660d1-525">Martin Fowler, " [simulações não são stubs](https://martinfowler.com/articles/mocksArentStubs.html)"</span><span class="sxs-lookup"><span data-stu-id="660d1-525">Martin Fowler, “ [Mocks Aren’t Stubs](https://martinfowler.com/articles/mocksArentStubs.html)”</span></span>
-   <span data-ttu-id="660d1-526">Martin Fowler, " [duplicata de teste](https://martinfowler.com/bliki/TestDouble.html)"</span><span class="sxs-lookup"><span data-stu-id="660d1-526">Martin Fowler, “ [Test Double](https://martinfowler.com/bliki/TestDouble.html)”</span></span>
-   [<span data-ttu-id="660d1-527">MOQ</span><span class="sxs-lookup"><span data-stu-id="660d1-527">Moq</span></span>](https://code.google.com/p/moq/)

### <a name="biography"></a><span data-ttu-id="660d1-528">Sumi</span><span class="sxs-lookup"><span data-stu-id="660d1-528">Biography</span></span>

<span data-ttu-id="660d1-529">Scott Allen é membro da equipe técnica da Pluralsight e do fundador da OdeToCode.com.</span><span class="sxs-lookup"><span data-stu-id="660d1-529">Scott Allen is a member of the technical staff at Pluralsight and the founder of OdeToCode.com.</span></span> <span data-ttu-id="660d1-530">Em 15 anos de desenvolvimento de software comercial, Scott trabalhou em soluções para tudo, desde dispositivos incorporados de 8 bits até aplicativos Web ASP.NET altamente escalonáveis.</span><span class="sxs-lookup"><span data-stu-id="660d1-530">In 15 years of commercial software development, Scott has worked on solutions for everything from 8-bit embedded devices to highly scalable ASP.NET web applications.</span></span> <span data-ttu-id="660d1-531">Você pode alcançar Scott em seu blog em OdeToCode ou no Twitter em [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode).</span><span class="sxs-lookup"><span data-stu-id="660d1-531">You can reach Scott on his blog at OdeToCode, or on Twitter at [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode).</span></span>
